...
CommanderClient::CommanderClient(spdlog::logger& logger)
    : logger_(logger), _socket(_io_context, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), 0)) {
}
...
void CommanderClient::Start() {
    if (!_is_configured) {
        logger_.error("Server URI not configured.");
        return;
    }
...
void CommanderClient::StartReceiveLoop() {
    ...
    _recv_thread = std::thread([this]() {
        while (_running && _socket.is_open()) {
            ...
            if (ec) {
                if (!_running) break;
                logger_.warn("Receive error: " + ec.message());
                continue;
            }
            ...
            if (method == 8) {
                ...
                if (it != _pending_requests.end()) {
                    ...
                    logger_.debug("Ignored packet: method={}", method);
                }
            }
        }
    });
...
void CommanderClient::StartPingLoop() {
    ...
    _ping_thread = std::thread([this]() {
        while (_running && _socket.is_open()) {
            ...
            if (ec) {
                logger_.error("Failed to send ping: {}", ec.message());
            } else {
                logger_.debug("Sent ping to server");
            }
...
void CommanderClient::HandlePing() {
    ...
    if (ec) {
        logger_.error("Failed to send pong: {}", ec.message());
    } else {
        logger_.debug("Sent pong to server");
    }
}
...
void CommanderClient::HandlePong() {
    ...
    logger_.debug("Received pong from server, latency={}ms", duration);

    // Check if pong was received within timeout (e.g., 1 second)
    if (duration > 1000) {
        logger_.warn("Pong received too late, latency={}ms", duration);
    }
}
...
std::vector<uint8_t> CommanderClient::SendRequest(uint8_t method, const std::vector<uint8_t>& payload, int timeout_ms) {
    ...
    if (future.wait_for(std::chrono::milliseconds(timeout_ms)) == std::future_status::timeout) {
        std::lock_guard<std::mutex> lock(_pending_mutex);
        _pending_requests.erase(method);
        logger_.warn("Timeout waiting for method {}", method);
        return {};
    }
...
void CommanderClient::Click() {
    ...
    if (resp.size() != 2 || resp[0] != 2 || resp[1] != 0xFF) {
        logger_.warn("Invalid Click ACK");
    }
}
...
std::string CommanderClient::Version() {
    ...
    if (resp.size() < 5 || resp[0] != 3) {
        logger_.error("Invalid Version response");
        return "";
    }
    uint32_t len;
...
void CommanderClient::SubscribeButtonStates() {
    ...
    if (ec) logger_.error("Failed to subscribe to button states {}", ec.message());
}
...
void CommanderClient::SetConfig(const ::capkfa::RemoteConfig& config) {
    ...
    if (!ParseServerUri(config.mouse_server().uri(), ip, port)) {
        logger_.error("Invalid server URI: {}", config.mouse_server().uri());
        return;
    }
    ...
    try {
        ...
        logger_.error("Failed to configure server: {}", e.what());
...
void CommanderClient::HandleButtonStateStream(const std::vector<uint8_t>& data) {
    ...
    for (size_t i = 5; i + 1 < 5 + len; i += 2) {
        ...
        if (_button_states[id] != pressed) {
            ...
            logger_.debug("Button {} state changed to {}", id, (pressed ? "pressed" : "released"));
